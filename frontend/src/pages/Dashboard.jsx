        ...
        <div className="mt-12 bg-card border border-border rounded-2xl p-8">
          ...
          <div className="space-y-3">
            {[
              ...
              { day: 20, label: 'RSS Feed + Shareable Links', done: true },
              { day: 21, label: 'Landing Page + Ship', done: false },
            ].map(item => (
              <div key={item.day} className="flex items-center gap-4">
                <span className={`font-mono text-xs w-12 ${item.done ? 'text-accent' : 'text-muted'}`}>Day {item.day}</span>
                <div className={`flex-1 h-px ${item.done ? 'bg-accent/30' : 'bg-border'}`} />
                <span className={`font-body text-sm ${item.done ? 'text-ink font-medium' : 'text-muted'}`}>{item.label}</span>
                {item.done && (
